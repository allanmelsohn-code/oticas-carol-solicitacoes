...
import { RefreshCw, Save, Users, UserPlus, Edit, Trash2, X, AlertTriangle } from 'lucide-react';
...
export function UserAdmin({ currentUser }: UserAdminProps) {
  ...
  const clearAllRequests = async () => {
    const confirmation = window.prompt(
      'Esta ação irá DELETAR PERMANENTEMENTE todas as solicitações e aprovações do sistema.\n\n' +
    ...
    try {
      const response = await fetch(`${API_BASE}/admin/clear-requests`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
          'X-Session-ID': getSessionId()
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to clear requests');
      }
      ...
      setError(`❌ ${err instanceof Error ? err.message : 'Erro ao limpar solicitações'}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="bg-orange-50 border-orange-200">
        ...
        <CardContent>
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="text-sm text-orange-900 font-medium">Limpar Todas as Solicitações</p>
              <p className="text-xs text-orange-700 mt-1">
                Remove permanentemente todas as solicitações e aprovações do sistema
              </p>
            </div>
            <Button 
              ...
              disabled={loading}
              variant="outline"
              className="bg-white hover:bg-red-50 text-red-700 border-red-300 hover:border-red-400"
            >
