...
import { CheckCircle, Database, Loader2, AlertCircle, Wifi } from 'lucide-react';
...
export function Setup() {
  ...
  const testBackendHealth = async () => {
    ...
    try {
      ...
      setHealthStatus('offline');
      setError(`Erro ao conectar ao backend: ${err.message}. Verifique se a Edge Function está deployada no Supabase.`);
    }
  };
          ...
          <CardContent className="space-y-6">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <p className="text-sm text-yellow-800 mb-3">
                Antes de executar o setup, teste se o backend está respondendo:
              </p>
              <Button 
              ...
              >
                {healthStatus === 'checking' ? (
                  ...
                  <>
                    ...
                    Backend Offline - Testar Novamente
                  </>
                ) : (
                  <>
              ...
              {healthStatus === 'online' && (
                <div className="mt-3 bg-green-50 border border-green-200 rounded p-3 text-sm text-green-800">
                  ✅ Backend está funcionando! Você pode executar o setup.
                </div>
              )}
              
              {healthStatus === 'offline' && (
                <div className="mt-3 bg-red-50 border border-red-200 rounded p-3 text-sm text-red-800">
                  <p className="font-semibold mb-2">❌ Backend não está respondendo</p>
                  <p className="mb-2">Possíveis causas:</p>
                  <ul className="list-disc list-inside space-y-1 ml-2">
                    <li>Edge Function não foi deployada no Supabase</li>
                    <li>Edge Function está com erro de sintaxe</li>
                    <li>Problema de conexão com a internet</li>
                  </ul>
                  <p className="mt-2">
                    <strong>Solução:</strong> Verifique o arquivo <code className="bg-red-100 px-1 rounded">DIAGNOSTICO-BACKEND.md</code> para instruções detalhadas.
                  </p>
                </div>
              )}
            </div>
