...
// Middleware - CORS configurado para aceitar todas as origens e métodos
app.use('*', cors({
  origin: '*',
...
app.get("/users", async (c) => {
  try {
    ...
    console.log('📋 GET /users - Authenticated user:', user?.email, 'Role:', user?.role);
    
    if (!user) {
      console.log('❌ GET /users - No user authenticated');
      return c.json({ error: 'Não autenticado. Faça login novamente.' }, 401);
    }
    
    if (user.role !== 'approver') {
      console.log('❌ Access denied - user role:', user?.role);
      console.log('❌ GET /users - User is not approver. Role:', user.role);
      return c.json({ error: 'Apenas administradores podem gerenciar usuários' }, 403);
    }
